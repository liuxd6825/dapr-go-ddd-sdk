...
func (s *High) DoAction(ctx context.Context, tx *model.Record, txIndex int, accTxs *model2.AccountRecords, result *action.AnalyseResult) {
	...
	switch s.rule.Period {
	case model2.SuFreqHighPeriod_Quarter:
		quarter := (tx.Date.Month()-1)/3 + 1
		periodKey = fmt.Sprintf("%d-Q%d", tx.Date.Year(), quarter)
	case model2.SuFreqHighPeriod_Year:
		periodKey = fmt.Sprintf("%d", tx.Date.Year())
	case model2.SuFreqHighPeriod_Month:
...
func (s *High) Done(accTxs *model2.AccountRecords, result *action.AnalyseResult) {
	highThreshold := s.rule.Threshold
	...
	s.frequencyCounter.Range(func(key, value interface{}) bool {
		...
		if count > s.rule.Threshold {
			...
			result.AddBatch(reason, model2.SuType_FreqHigh, txList.Records...)
		}
