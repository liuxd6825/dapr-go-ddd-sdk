...
func (d *GraphDao) AddNodeList(target map[string]*model2.NodeView, source []any) {
	for _, item := range source {
		if dbNode, ok := item.(dbtype.Node); ok {
			node := model2.NewNodeView()
...
func (d *GraphDao) AddRelationList(target map[string]*model2.RelationView, source []any) {
	for _, item := range source {
		if dbRel, ok := item.(dbtype.Relationship); ok {
			rel := model2.NewRelationView()
