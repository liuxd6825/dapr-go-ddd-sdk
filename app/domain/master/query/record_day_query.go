...
type RecordDayFindBySumChartQuery struct {
	...
	Filter      string `json:"filter"  param:"filter" validate:"required" title:"过滤条件"`
}

type RecordDayFindBySumTableQuery struct {
	CaseId      string `json:"caseId"  param:"caseId" validate:"required" title:"项目ID"`
	Filter      string `json:"filter"  param:"filter" validate:"required" title:"过滤条件"`
	GroupFilter string `json:"groupFilter"  param:"groupFilter"  title:"分组条件"`
	Sort        string `json:"sort"  param:"sort" title:"排序"`
	PageSize    int64  `json:"pageSize"  param:"pageSize" title:"分页大小"`
	PageNum     int64  `json:"pageNum"  param:"pageNum"  title:"分页页号"`
}
