...
func NewAnalyseResult(taskId string, account string) *AnalyseResult {
	return &AnalyseResult{
		Account: account,
		Records: make(map[string]*model.SuRecord),
		Batches: []*AnalyseBatch{},
	}
}

func (s *AnalyseResult) AddBatch(name string, suType model.SuType, records ...*model.Record) *AnalyseBatch {
	batch := &AnalyseBatch{
		Name:    name,
		Type:    suType,
		Records: records,
...
func (s *AnalyseResult) AddRecord(tx *model.Record, suType model.SuType, reason string) *model.SuRecord {
	var item *model.SuRecord
	...
	item.Reasons = append(item.Reasons, model.SuRecordReason{
		Reason: reason,
		Type:   suType,
	})
	return item
