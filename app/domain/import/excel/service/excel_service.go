...
import (
	...
	"github.com/liuxd6825/dapr-go-ddd-sdk/app/domain/import/excel/command"
...
type ExcelViewService struct {
	fileService  *FileService
	sheetService *SheetService
	rowService   *RowService
	fsService    *service.FileService
}

func NewExcelViewService() *ExcelViewService {
	return &ExcelViewService{
		fileService:  NewFileService(),
		sheetService: NewSheetService(),
		rowService:   NewRowService(),
		fsService:    service.NewFileService(),
	}
}

func (s *ExcelViewService) CreateFile(ctx context.Context, cmd *command.CreateFileAppCmd) (*model.ExcelFile, error) {
...
func (s *ExcelViewService) CreateRow(ctx context.Context, cmd *command.CreateRowsAppCmd) error {
	if err := cmd.Validate(); err != nil {
		return err
	}

	bytes, err := s.readFile(cmd.FileId)
	if err != nil {
		return err
	}
...
func (s *ExcelViewService) readFile(fileName string) ([]byte, error) {
	bytes, err := s.fsService.ReadFile(fileName)
	return bytes, err
}
...
// @return error
func (s *ExcelViewService) FindRows(ctx context.Context, cmd *command.FindRowsAppQuery) (*command.FindRowsAppResult, bool, error) {
	if err := cmd.Validate(); err != nil {
...
func (s *ExcelViewService) FindFile(ctx context.Context, qry *command.FindFileAppQuery) (*model.ExcelFile, error) {
...
/*
func (e *ExcelViewService) FindRecords(ctx context.Context, appCmd *command.FindRecordsAppQuery) ([]*model2.RecordIe, error) {
	if err := appCmd.Validate(); err != nil {
		return nil, err
	}
