...
import (
	"context"
	"fmt"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/ai/rag/light_rag"
	"net/http"
	"sync"
	"time"
...
type DocumentService struct {
	dao      *dao.DocumentDao
	graphRag *light_rag.DocumentService
...
func NewDocumentService(statusProvider interfaces.ImportStatusProvider) *DocumentService {
	documentServiceOnce.Do(func() {
		ragCfg := light_rag.LoadLightRagConfig()
		cli := light_rag.NewClient(ragCfg.Url, ragCfg.ApiKey, &http.Client{})
		documentService = &DocumentService{
			dao: dao.NewDocumentDao(DBKey),
			//graphRag:       NewGraphRag(),
			//docExtract:     doc_extract.NewExtract(logs.GetLogger2()),
			statusProvider: statusProvider,
			graphRag:       light_rag.NewDocumentService(cli),
		}
	...
	gp.Try(func() error {
		...
		for {
			...
			for _, doc := range list {
				...
				trackId, err := s.graphRag.Upload(ctx, fileName, fs)
				if err != nil {
				...
				for {
					...
					resp, err := s.graphRag.GetStatuses(ctx, trackId)
					if err != nil {
						_ = s.updateState(ctx, doc, model.DocumentState_Failure, err.Error())
						success = false
						break
					}
					if len(resp.Statuses.FAILED) > 0 {
						doc.RagDocId = resp.Statuses.FAILED[0].Id
						_ = s.updateState(ctx, doc, model.DocumentState_Failure, "文档解析失败")
						success = false
						break
					}
					if len(resp.Statuses.COMPLETED) > 0 {
						doc.RagDocId = resp.Statuses.COMPLETED[0].Id
						break
					}
					if len(resp.Statuses.PROCESSED) > 0 {
						doc.RagDocId = resp.Statuses.PROCESSED[0].Id
						break
					}
				}
				if !success {
...
func (s *DocumentService) updateState(ctx context.Context, doc *model.Document, state model.DocumentState, msg string) error {
	return gp.Try(func() error {
		...
		if doc.RagDocId != "" {
			callOptions.SetUpdateFields([]string{"state", "message", "rag_doc_id"})
		} else {
...
func (s *DocumentService) delete(ctx context.Context, caseId, id string, opts ...idao.CallOptions) error {
	...
	doc, err := s.dao.FindById(ctx, id)
	if err != nil {
		return err
	}
	res := s.dao.DeleteByRSQL(ctx, fmt.Sprintf(`tenant_id=="%s" and case_id=="%s" and id=="%s"`, tenantId, caseId, id), opts...)
	if res.Error != nil {
		return res.Error
	}
	if doc.RagDocId != "" {
		_, err = s.graphRag.Delete(ctx, []string{doc.RagDocId})
		if err != nil {
			return err
		}
	}
...
func (s *DocumentService) DeleteByRSql(ctx context.Context, cmd *command.DocumentDeleteByDocCommand, opts ...idao.CallOptions) error {
	docs, err := s.dao.FindByRSQL(ctx, fmt.Sprintf(`source_type=="document" and source_id=="%s"`, cmd.Data.DocId))
	if err != nil {
		return err
	}
	if len(docs) == 0 {
	...
	return s.delete(ctx, docs[0].CaseId, docs[0].Id)
}
