...
func NewQueryTemplate(tplSet *pongo2.TemplateSet, serverFs afero.Fs, webFs afero.Fs) *QueryTemplate {
...
func (t *QueryTemplate) Execute(ctx context.Context, sch *jsonschema.Schema) (string, error) {
	data := map[string]any{
		"schema":     sch,
		"sv":         sch.GetView(),
	...
	fsData, err := afero.ReadFile(t.webFs, "/@tpl/query.tpl.html")
	if err != nil {
		return "", err
	}
	tpl, err := t.tplSet.FromBytes(fsData)
	if err != nil {
		panic("NewQueryTemplate() " + err.Error())
	}
...
func (t *QueryTemplate) Render(htmlFile string, schemaFile string) (template.HTML, error) {
	/*
