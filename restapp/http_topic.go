...
func (s *HttpServer) addSubscription(sub *common.Subscription, fn common.TopicEventHandler) error {
	subscribes := make([]*ddd.Subscribe, 1)
	subscribes[0] = &ddd.Subscribe{
...
func (s *HttpServer) AddTopicEventHandler(sub *common.Subscription, fn common.TopicEventHandler) error {
	if sub == nil {
	...
	if err := s.addSubscription(sub, fn); err != nil {
		return err
	}

	s.app.HandleMany("", sub.Route, optionsHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
