...
func New(server element2.Server) *Feign {
	return newFeign(server)
}

func newFeign(server element2.Server) *Feign {
...
func (f *Feign) NewProxy(vm *goja.Runtime, workPath string) *element2.Proxy {
	return element2.NewProxy(f.server, vm, workPath, f.values)
}
