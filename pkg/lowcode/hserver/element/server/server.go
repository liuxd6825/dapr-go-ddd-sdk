...
import (
	...
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/lowcode/hserver/element/funcs"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/lowcode/hserver/pkg/console_pkg"
...
type Server struct {
	...
	services     *types.CMap[element2.Service] // 服务Map
	env          *env.Env                      // 环境变量
...
func NewServer(httpServer *restapp.HttpServer, srcFileName string, srcFs afero.Fs, factory element2.Factory, env *env.Env, opts ...element2.NewServerOptions) (element2.Server, error) {
	...
	server := &Server{
		...
		app:          app,
		services:     types.NewCMap[element2.Service](),
		fsPkg:        fsPkg,
		env:          env,
		srcFs:        srcFs,
		cacheEnable:  true,
...
func (s *Server) Factory() element2.Factory {
	return s.factory
}
...
func (s *Server) start() error {
	...
	runValue := &element2.ApiRunValues{
		WorkPath: s.WorkPath(),
		Self:     s,
		Server:   s,
	}
...
//	@param serviceName
//	@return *Service
func (s *Server) GetService(apiServiceName string) element2.Service {
...
func (s *Server) Init(opts *element2.ServerInitOptions) {
	if opts == nil {
		return
	}
