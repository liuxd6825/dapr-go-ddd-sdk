...
type Manager struct {
	funcs     *types.CMap[element2.Func]
...
func NewFuncManager(logger logrus.FieldLogger, reader fs.Reader) *Manager {
	return &Manager{
		funcs:  types.NewCMap[element2.Func](),
		logger: logger,
		reader: reader,
	}
}

func (b *Manager) Get(funcName string) (fun element2.Func, ok bool) {
	fun, ok = b.funcs.Get(funcName)
	return fun, ok
}

func (b *Manager) Items() map[string]element2.Func {
	return b.funcs.Items()
}

func (b *Manager) Add(fun element2.Func) error {
...
//	@return res
//	@return err
func (b *Manager) Run(ctx context.Context, funcName string, runValues *element2.ApiRunValues, checkHave bool, opts ...RunOptions) (res any, err error) {
	srcFileName := ""
	...
	opts = append(opts, func(vm *goja.Runtime) error {
		...
		pkgObj := goja.NewSharedDynamicObject(element2.NewProxy(runValues.Server, vm, runValues.WorkPath, pkgMap))
...
func SetRunValues(vm *goja.Runtime, runValues *element2.ApiRunValues, data ...map[string]any) error {
	if runValues != nil {
		...
		if runValues.Self != nil {
			self, ok := runValues.Self.(element2.Self)
			if ok {
