...
import (
	...
	jsonutils2 "github.com/liuxd6825/dapr-go-ddd-sdk/pkg/utils/jsonutils"
...
func (c *WebContext) ReadMap(sch *jsonschema.Schema) map[string]any {
	...
	bytes := c.ReadBytes()
	val, err := jsonutils2.UnmarshalTime(bytes, &jsonutils2.UnmarshalTimeOptions{
	...
	if dataVal, ok := object["data"]; ok {
		if data, ok := dataVal.(map[string]any); ok {
			if timeVal, ok := data["birthday"]; ok {
				if t, ok := timeVal.(*times2.Date); ok {
					utc := t.Time()
...
func (c *WebContext) SetError(errOrMsg any, httpStatus ...int) {
	...
	if e, ok := errOrMsg.(error); ok {
		...
		msgData, err := jsonutils2.MarshalBytes(data)
		if err != nil {
...
var parseTime = func(val string, key any) (timeVal any, err error) {
	...
	if sch, ok := key.(*jsonschema.Schema); ok {
		if sch.Types.Contains(jsonschema.JsonType_DateTimeType) {
			if tm, err := times2.AsTime(val); err != nil {
				return nil, err
			} else if tm != nil {
				timeVal = times2.GetTime(tm.PTime())
			}
		} else if sch.Types.Contains(jsonschema.JsonType_DateType) {
			if tm, er := times2.AsTime(val); er != nil {
				return nil, er
			} else if tm != nil {
				timeVal = times2.GetDate(tm.PTime())
			}
		}
	}
	return timeVal, nil
