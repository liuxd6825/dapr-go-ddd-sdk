...
type IMongoDao[T any] interface {
	store2.IStore[T]
...
type Dao[T any] struct {
	...
	base       *store2.BaseDao[T]
	eb         store2.EntityBuilder[T]
}

func NewDao[T any](dbSch *store2.DBSchema, initFun func(ctx context.Context) (mongodb IMongoDB, collection *mongo.Collection), opts ...*Options[T]) store2.IStore[T] {
	return NewMongoDao(dbSch, initFun, opts...)
}

func NewMongoDao[T any](dbSch *store2.DBSchema, initFun func(ctx context.Context) (mongodb IMongoDB, collection *mongo.Collection), opts ...*Options[T]) *Dao[T] {
	if dbSch == nil {
	...
	if eb == nil {
		eb = store2.NewAnyEntityBuilder[T](dbSch)
	}
	...
	base := store2.NewBaseDao(eb, dbSch)
	r.base = base
	r.eb = base.GetEb()
...
func (r *Dao[T]) GetSchema() *store2.DBSchema {
...
func (r *Dao[T]) Save(ctx context.Context, data *ddd.SetData[T], opts ...store2.Options) (setResult *store2.SetResult[T]) {
	var err error
	defer func() {
...
func (r *Dao[T]) Merge(ctx context.Context, entity T, fields map[string]string, opts ...store2.Options) (setResult *store2.SetResult[T]) {
	panic("implement me")
}

func (r *Dao[T]) BulkWrite(ctx context.Context, models []mongo.WriteModel, opts ...store2.Options) (*store2.BulkWriteResult, error) {
	if len(models) == 0 {
		return &store2.BulkWriteResult{}, nil
	}
...
func (r *Dao[T]) newBulkWriteResult(bulkRes *mongo.BulkWriteResult) *store2.BulkWriteResult {
	res := store2.NewBulkWriteResult()
	if bulkRes != nil {
...
func (r *Dao[T]) UpdateManyMaskById(ctx context.Context, entities []T, mask []string, opts ...store2.Options) *store2.SetResult[T] {
...
func (r *Dao[T]) GetConfig() *store2.EntityBuilderConfig {
...
func (r *Dao[T]) db2entity222(data map[string]any) T {
	...
	if isMap {
		...
		for _, field := range r.GetSchema().Fields {
			...
			if field.DataType == store2.DataType_Date || field.DataType == store2.DataType_Time {
				if pDate, ok := val.(primitive.DateTime); ok {
					timeVal := pDate.Time().In(times.GetLocalTimeZone())
					val = &timeVal
				}
			} else if field.DataType == store2.DataType_Array {
