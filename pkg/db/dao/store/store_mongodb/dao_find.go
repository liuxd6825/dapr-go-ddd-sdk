...
import (
	...
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/utils/mongoutils"
...
func (r *Dao[T]) FindOneAndUpdateById(ctx context.Context, tenantId string, id string, data map[string]any, opts ...store2.Options) (T, error) {
	var null T
...
func (r *Dao[T]) FindById(ctx context.Context, tenantId string, id string, opts ...store2.Options) *store2.FindOneResult[T] {
...
func (r *Dao[T]) FindByIds(ctx context.Context, tenantId string, ids []string, opts ...store2.Options) *store2.FindListResult[T] {
...
func (r *Dao[T]) FindOneByMap(ctx context.Context, tenantId string, filterMap map[string]interface{}, opts ...store2.Options) *store2.FindOneResult[T] {
...
func (r *Dao[T]) FindListByMap(ctx context.Context, tenantId string, filterMap map[string]interface{}, opts ...store2.Options) *store2.FindListResult[T] {
	return r.DoFindList(func() ([]T, bool, error) {
		var list []T
		filter := r.NewFilter(tenantId, filterMap)
		findOptions := getFindOptions(opts...)
...
func (r *Dao[T]) FindListByBsonM(ctx context.Context, tenantId string, filter bson.M, opts ...store2.Options) *store2.FindListResult[T] {
	return r.DoFindList(func() ([]T, bool, error) {
		findOptions := getFindOptions(opts...)
		list, count, err := r.mFindList(ctx, filter, findOptions)
		return list, count, err
...
func (r *Dao[T]) FindByRSQL(ctx context.Context, tenantId string, rsql string, opts ...store2.Options) *store2.FindListResult[T] {
...
func (r *Dao[T]) FindAll(ctx context.Context, tenantId string, opts ...store2.Options) *store2.FindListResult[T] {
	return r.FindListByMap(ctx, tenantId, nil, opts...)
}

func (r *Dao[T]) findPaging(ctx context.Context, query store2.FindPagingQuery, opts ...store2.Options) store2.FindPagingResult[T] {
	tenantId := appctx.GetTenantId2(ctx)
	return r.doFilter(tenantId, query.GetFilter(), func(filter *rsql_mongo.Filter) (store2.FindPagingResult[T], bool, error) {
		if err := assert2.NotEmpty(tenantId, assert2.NewOptions("tenantId is empty")); err != nil {
			return nil, false, err
		}
		ctx = r.getSessionCtx(ctx)
		...
		findData := store2.NewFindPagingResult[T](data, inOut.totalRows, query, err)
...
type findOption struct {
	...
	query     store2.FindPagingQuery // 分页查询条件
	results   any                    // 返回数据
	totalRows int64                  // 返回记录数
}

func newFindOption(filter *rsql_mongo.Filter, query store2.FindPagingQuery, results any) *findOption {
	return &findOption{
		filter:    filter,
		query:     query,
		results:   results,
...
func (r *Dao[T]) newFindOptions(query store2.FindPagingQuery, opts ...store2.Options) (*mongo_options.FindOptions, error) {
	findOptions := getFindOptions(opts...)
	if query == nil {
...
func (r *Dao[T]) getFindOptionsProjection(query store2.FindPagingQuery) bson.D {
...
func (r *Dao[T]) FindAutoComplete(ctx context.Context, qry store2.FindAutoCompleteQuery, opts ...store2.Options) store2.FindPagingResult[T] {
	f := store2.NewFindPagingQuery()
	groupCols := []*store2.GroupCol{
...
func (r *Dao[T]) FindDistinct(ctx context.Context, qry store2.FindDistinctQuery, opts ...store2.Options) store2.FindPagingResult[T] {
	f := store2.NewFindPagingQuery()
...
func (r *Dao[T]) AggregateByPipeline(ctx context.Context, pipeline mongo.Pipeline, data interface{}, opts ...store2.Options) error {
	sCtx := r.getSessionCtx(ctx)
...
func (r *Dao[T]) CopyTo(ctx context.Context, tenantId string, rsql string, toCollectionName string, opts ...store2.Options) error {
	options := getAggregateOptions(opts...)
...
func (r *Dao[T]) SumEntity(ctx context.Context, qry store2.FindPagingQuery, opts ...store2.Options) ([]T, bool, error) {
	data := r.NewEntityList()
	sCtx := r.getSessionCtx(ctx)
...
func (r *Dao[T]) SumMap(ctx context.Context, qry store2.FindPagingQuery, opts ...store2.Options) ([]map[string]any, bool, error) {
	data := make([]map[string]any, 0)
	_, found, err := r.SumByQuery(r.getSessionCtx(ctx), qry, &data, opts...)
	return data, found, err
}

func (r *Dao[T]) SumByQuery(ctx context.Context, qry store2.FindPagingQuery, data any, opts ...store2.Options) (any, bool, error) {
	...
	f3 := ""
	mustWhere, ok := qry.(store2.FindPagingQueryMustWhere)
	if ok {
...
func (r *Dao[T]) SumByRSQL(ctx context.Context, tenantId string, rSql string, valueCols []*store2.ValueCol, opts ...store2.Options) map[string]any {
	filter, err := r.getFilter(tenantId, rSql)
	if err != nil {
		panic(err)
	}
...
func (r *Dao[T]) sum(ctx context.Context, filter *rsql_mongo.Filter, valueCols []*store2.ValueCol, list any, opts ...store2.Options) (any, bool, error) {
	coll := r.getCollection(ctx)
	/*
	...
	*/
	...
	for _, col := range valueCols {
		...
		if aggFunc == store2.AggFuncCount.Name() {
			field = "count_" + field
			aggFunc = "$sum"
			fieldValue = 1
		} else {
	...
	if err != nil {
		result := store2.NewFindPagingResultWithError[T](err)
...
func (r *Dao[T]) CountByMap(ctx context.Context, tenantId string, filterData any, opts ...store2.Options) (int64, error) {
...
func (r *Dao[T]) CountByRSQL(ctx context.Context, tenantId string, rsql string, opts ...store2.Options) (int64, error) {
...
func (r *Dao[T]) doList(tenantId, rsql string, fun func(filter *rsql_mongo.Filter) ([]T, bool, error)) *store2.FindListResult[T] {
	if err := assert2.NotEmpty(tenantId, assert2.NewOptions("tenantId is empty")); err != nil {
	...
	return store2.NewFindListResult(data, ok, err)
}

func (r *Dao[T]) doFilter(tenantId, rsql string, fun func(filter *rsql_mongo.Filter) (store2.FindPagingResult[T], bool, error)) store2.FindPagingResult[T] {
	if err := assert2.NotEmpty(tenantId, assert2.NewOptions("tenantId is empty")); err != nil {
...
func (r *Dao[T]) DoFindList(fun func() ([]T, bool, error)) *store2.FindListResult[T] {
	...
	return store2.NewFindListResult[T](data, isFound, err)
}

func (r *Dao[T]) DoFindOne(fun func() (T, bool, error)) *store2.FindOneResult[T] {
	data, isFound, err := fun()
	return store2.NewFindOneResult[T](data, isFound, err)
}

func (r *Dao[T]) DoSet(fun func() (T, error)) *store2.SetResult[T] {
	data, err := fun()
	return store2.NewSetResult[T]().SetError(err).SetData(data)
}

func (r *Dao[T]) StartTx(ctx context.Context, txFun store2.TxFunc, options ...*store2.SessionOptions) (err error) {
...
*/
func (r *Dao[T]) FindPaging(ctx context.Context, qry store2.FindPagingQuery, opts ...store2.Options) (result store2.FindPagingResult[T]) {
	defer func() {
		if e := recover(); e != nil {
	...
	var findData store2.FindPagingResult[T]
	var err error
	//data := r.NewEntityList()
	...
	findData = store2.NewFindPagingResult[T](data, groupQueryResult.totalRows, qry, err)
	...
	if err != nil || errt != nil {
		return store2.NewFindPagingResultWithError[T](err, errt)
	}
	...
	findData = store2.NewFindPagingResult[T](data, totalRows, qry, err)
	// 进行汇总计算
	if len(qry.GetValueCols()) > 0 {
		sumData, _, err := r.SumEntity(ctx, qry, opts...)
		findData.SetSumData(sumData)
...
func (r *Dao[T]) find(ctx context.Context, fOpt *findOption, opts ...store2.Options) error {
	filter := fOpt.filter
...
func (r *Dao[T]) findByGroupQuery(ctx context.Context, qry *QueryGroup, results *findByGroupQueryOptions, opts ...store2.Options) error {
	coll := r.getCollection(ctx)
