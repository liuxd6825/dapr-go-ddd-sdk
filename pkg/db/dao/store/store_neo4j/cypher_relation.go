...
func (c *relationCypher[T]) Sum(ctx context.Context, tenantId, rSQL string, valueCols []*store2.ValueCol) (CypherResult, error) {
	// 解析rsql
	process := rsql_neo4j.NewProcess(tenantId, "r")
	if err := rsql.ParseProcess(rSQL, process); err != nil {
		return nil, err
...
func (c *relationCypher[T]) FindPaging(ctx context.Context, qry store2.FindPagingQuery) (CypherResult, error) {
...
func (c *relationCypher[T]) getFieldName(field *store2.Field) string {
	return field.DBName
}
