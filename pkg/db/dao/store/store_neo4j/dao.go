...
func NewNodeDao[T any](driver neo4j.DriverWithContext, config *Config[T], opts ...*Options[T]) store2.IStore[T] {
...
func NewRelationDao[T any](driver neo4j.DriverWithContext, config *Config[T], opts ...*Options[T]) store2.IStore[T] {
...
func NewDao[T any](driver neo4j.DriverWithContext, cypher Cypher[T], config *Config[T], opts ...*Options[T]) store2.IStore[T] {
	return newDao(driver, cypher, config, opts...)
}
...
func (d *Dao[T]) DoFilter(ctx context.Context, tenantId string, fun func() (store2.FindPagingResult[T], bool, error), opts ...store2.Options) store2.FindPagingResult[T] {
	data, _, err := fun()
	if err != nil {
...
func (d *Dao[T]) DoList(ctx context.Context, tenantId string, fun func() *store2.FindListResult[T], opts ...store2.Options) *store2.FindListResult[T] {
	data := fun()
	return data
}

func (d *Dao[T]) newSetManyResult(ctx context.Context, result *Neo4jResult[T], err error) *store2.SetResult[T] {
	if err != nil {
		return store2.NewSetResultError[T](err)
	}
	var data []T
	if err := result.GetList(ctx, "n", &data, d.config.DBSchema); err != nil {
		store2.NewSetResultError[T](err)
	}
	return store2.NewSetResultEmpty[T]()
}

func (d *Dao[T]) DoSet(ctx context.Context, tenantId string, cypher string, params map[string]interface{}, opts ...store2.Options) (*Neo4jResult[T], error) {
...
func (d *Dao[T]) Run(ctx context.Context, cypher string, params map[string]any, isWriteMode bool, opts ...store2.Options) (*Neo4jResult[T], error) {
...
func (d *Dao[T]) DeleteMany(ctx context.Context, tenantId string, entity []T, opts ...idao.CallOptions) *store2.SetResult[T] {
	return store2.NewSetResult[T]()
}
