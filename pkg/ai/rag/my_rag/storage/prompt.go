...
- strength: a numeric score indicating strength of the relationship between the source entity and target entity (use a number between 1-10)
- keywords: one or more high-level key words that summarize the overarching nature of the relationship, focusing on concepts or themes rather than specific details
...
4. Format your output as a VALID JSON object with the following structure:
{
  "entities": [
    {
      "name": string,
      "type": string (one of the provided entity types ONLY),
      "description": string
    }
  ],
  "relationships": [
...
Output:
{
  ...
  "relationships": [
    ...
    {{if $j}},{{end}}
    {
      ...
      "entity": "{{$output.TargetEntity}}",
      "description": "{{$output.Description}}",
      "keywords": [{{range $k, $v := $output.Keywords}}{{if $k}}, {{end}}"{{$v}}"{{end}}],
