...
func (h *RenderHandler) IsFileExist(fs afero.Fs, path string) (bool, error) {
	exists, err := afero.Exists(fs, path)
	return exists, err
}

// isDynamicPageRegex 使用正则表达式检查 HTML 文件是否是动态页面
func (h *RenderHandler) isDynamicPage(ctx context.Context, ictx iris.Context, fs afero.Fs, filePath string, prodMode bool) (bool, error) {
	// 在dev模式下，默认为后端渲染。
	if !h.prodMode {
		return true, nil
	}
