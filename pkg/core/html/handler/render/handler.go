...
import (
	...
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/core/html/handler/render/engine"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/env"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/errors"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/logs"
	"github.com/liuxd6825/dapr-go-ddd-sdk/pkg/lowcode/hserver/utils"
...
func RegisterRenderHandler(irisApp *iris.Application, env *env.Env, grValue map[string]any) {
	if isRegister {
		return
	}
	webFs, ok := env.Fsm.GetFs(env.App.WebName)
	if !ok {
		panic(fmt.Errorf(" %s fs not exists", env.App.WebName))
	}
	serverFs, err := localfs.NewFs(localfs.Config{
		Name: "server",
		Path: "./",
	})
	if err != nil {
	...
	handler := NewRenderHandler(irisApp, grValue, cfg)
	irisApp.Get("/{file:path}", handler.Handle)
	restapi.View = handler.RenderView
	return
}

func NewRenderHandler(app *iris.Application, globalValues map[string]any, cfg *Config) *RenderHandler {
	f := &RenderHandler{
		app:          app,
		cfg:          cfg,
	...
	en := engine.NewEngine(cfg.Env, cfg.ServerFs, cfg.WebFs, ".html")
...
func (h *RenderHandler) PathInterceptor(ctx iris.Context) {
...
func (h *RenderHandler) Handle(ictx iris.Context) {
	var err error
	defer func() {
...
func (h *RenderHandler) RenderView(ctx context.Context, ictx iris.Context, fileName string, viewData ...map[string]any) error {
	if fileName == "" {
...
func (h *RenderHandler) isFileExist(fileName string) (fs afero.Fs, resFileName string, exist bool, err error) {
...
func (h *RenderHandler) GetJsTsFile(fileName string) (fs afero.Fs, resFileName string, exist bool, err error) {
...
// @param viewData 数据
// @return error
func (h *RenderHandler) render(ctx context.Context, ictx iris.Context, fs afero.Fs, fileName string, viewData ...map[string]any) error {
	var err error
	isRender := false
...
func (h *RenderHandler) writeFile(ictx iris.Context, fs afero.Fs, fileName string) error {
...
//	@param path
//	@return error
func (h *RenderHandler) preloadDynamicPages(ctx context.Context, ictx iris.Context, path string) error {
	srcFs := h.cfg.WebFs
