...
func NewSession(isWrite bool, driver neo4j.DriverWithContext) store.Session {
...
func (r *Neo4jSession) UseTransaction(ctx context.Context, dbFunc store.SessionFunc, opts ...*store.SessionOptions) error {
	if dbFunc == nil {
		return nil
	}

	opt := store.NewSessionOptions(opts...)
