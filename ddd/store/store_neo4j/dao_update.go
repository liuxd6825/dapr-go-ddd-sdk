...
func (d *Dao[T]) UpdateByRSQL(ctx context.Context, tenantId, rSQL string, entity T, opts ...store.Options) *store.SetResult[T] {
...
func (d *Dao[T]) UpdateMany(ctx context.Context, tenantId string, list []T, opts ...store.Options) *store.SetResult[T] {
	res := store.NewSetResultEmpty[T]()
	gp.Try(func() error {
		for _, ent := range list {
